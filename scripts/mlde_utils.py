#!/usr/bin/env python3
"""
Shared utilities for the small-data MLDE workflow.

This module centralizes:
- feature-bank loading for learned embeddings
- simple positional sequence encodings (one-hot and BLOSUM62)
- CV helpers and metric computation
- standardized OOF / prediction table formatting
- picklable fitted-model artifacts used across scripts
"""
...
def conformal_q(abs_residuals: np.ndarray, alpha: float) -> float:
    scores = np.sort(np.asarray(abs_residuals).ravel())
    if len(scores) == 0:
        return float("nan")
...
def normalize_feature_source(name: str) -> str:
    ...
    if lowered in COMPUTED_FEATURE_SOURCES:
        return lowered
    return name.strip()


def load_embedding_npz(npz_path: str) -> Tuple[List[str], np.ndarray]:
...
def build_embedding_bank(npz_path: str, name: Optional[str] = None) -> EmbeddingBank:
    sequences, embeddings = load_embedding_npz(npz_path)
...
def resolve_embedding_paths(
    ...
    learned_sources = [normalize_feature_source(source) for source in feature_sources if normalize_feature_source(source) not in COMPUTED_FEATURE_SOURCES]
    paths: Dict[str, str] = {}
    ...
    if missing:
        raise ValueError(
            "Missing embedding paths for learned feature sources: "
            + ", ".join(sorted(missing))
        )
    return paths
...
def load_embedding_banks(
    ...
    return {name: build_embedding_bank(path, name=name) for name, path in resolved.items()}
...
def infer_sequence_length(sequences: Sequence[str], expected_length: Optional[int] = None) -> int:
    ...
    if expected_length is not None and length != expected_length:
        raise ValueError(
            f"Sequence length mismatch: expected {expected_length}, observed {length}"
        )
    return length


def encode_onehot_sequences(
...
def assemble_feature_matrices(
    ...
    for raw_source in feature_sources:
        ...
        if source == "onehot":
            matrices[source] = encode_onehot_sequences(
                sequences,
            ...
            continue
        if source == "blosum62":
            matrices[source] = encode_blosum62_sequences(
                sequences,
                expected_length=expected_sequence_length,
            )
            source_missing_counts[source] = 0
    ...
    if computed_len is None:
        computed_len = infer_sequence_length(
            sequences,
            expected_length=expected_sequence_length,
        )
...
def cross_val_predict_with_uncertainty(
    ...
    return compute_metrics(y, oof), oof, native_std, fold_ids, fold_rows
...
def prepare_supervised_dataframe(
    ...
    reducer = np.mean if replicate_policy == "mean_by_sequence" else np.median
    grouped = work.groupby(seq_col, sort=False, dropna=False)
    rows: List[pd.Series] = []
    for sequence, group in grouped:
        row = group.iloc[0].copy()
        row[target_col] = float(reducer(group[target_col].astype(float).values))
        row["is_aggregated_row"] = True
        row["n_source_rows"] = int(len(group))
        if id_col and id_col in group.columns:
            row["source_ids"] = "|".join(group[id_col].astype(str).tolist())
        rows.append(row)
